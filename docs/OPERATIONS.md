...
**Before running:** ensure `.env` secrets are backed up if needed; Git history is unaffected.

## Optional global Docker cleanup

Only if you accept removing unused images/containers network-wide on the host:

```bash
docker system prune -af
```

Not run automatically by this project.
