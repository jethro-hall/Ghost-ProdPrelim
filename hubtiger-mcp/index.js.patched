...
function normalizeAvailabilityInput(body) {
  ...
  const rawDays = Number(payload.days || body?.days || 1);
  const days = Math.max(1, Math.min(Number.isFinite(rawDays) ? rawDays : 1, 14));
  return {
    store: normalizeStoreSlug(payload.store || body?.store || ''),
    startDate: normalizeDateOnly(payload.start_date || payload.date || body?.start_date || body?.date || ''),
    days,
...
async function lookupTechnicianAvailableSlots(body) {
  ...
  const baseUrl = String(process.env.HUBTIGER_BASE_URL || 'https://hubtiger-api.azurewebsites.net').trim().replace(/\/$/, '');
  const partnerId = String(process.env.HUBTIGER_PARTNER_ID || '2186').trim();
  const code = String(process.env.HUBTIGER_FUNCTION_CODE || process.env.HUBTIGER_API_CODE || '').trim();
  ...
  if (!code) {
    return buildMcpLikeResult({
      ...
      startNs,
      cacheMode,
      error: 'availability_lookup_missing_function_code',
      data: {
        success: false,
        operation: 'availability_lookup',
        blocked: false,
        ...
        data: {
          error_code: 'availability_lookup_missing_function_code',
  ...
  try {
    for (let i = 0; i < input.days; i += 1) {
      const date = addUtcDays(input.startDate, i);
      if (!date) continue;

      const url = new URL('/api/Services/TechniciansAvailableSlots', baseUrl);
      url.searchParams.set('PartnerID', partnerId);
      url.searchParams.set('TechnicianID', String(resource.technicianId));
      url.searchParams.set('Date', date);
      url.searchParams.set('Services', '');
      ...
      upstreamStatuses.push({ date, status: response.status });
      ...
      const openSlots = Array.isArray(json.openSlots) ? json.openSlots : [];
      const daySlots = openSlots
        .map((slot) => ({
          date,
          available_slot: String(slot?.availableSlot || ''),
          display: formatAvailabilityDisplay(slot?.availableSlot),
          time: formatAvailabilityTime(slot?.availableSlot),
          technician_id: Number(slot?.technicianID || resource.technicianId),
        }))
      ...
      slotsByDate.push({
        ...
        first_slots: daySlots.slice(0, 5).map((slot) => slot.time),
  ...
  const voicePayload = {
    ...
    operation: 'availability_lookup',
    blocked: false,
    message: allSlots.length
      ? `I found ${allSlots.length} available workshop slots for ${resource.label}.`
      : `I could not see open workshop slots for ${resource.label} from ${input.startDate}.`,
    data: {
      store: input.store,
      ...
      first_slots: allSlots.slice(0, 5).map((slot) => slot.display),
      slots_by_date: slotsByDate,
      requires_staff_confirmation: true,
      source: 'hubtiger TechniciansAvailableSlots',
    },
  };
