        ...
        try {
            ...
            if (options.downloadFields) {
                ...
                returnData.push(...itemWithAttachments.map((item) => ({
                    ...item,
                    json: (0, utils_1.legacyFlattenOutput)(item.json, nodeVersion),
                })));
                continue;
            }
            const output = (0, utils_1.legacyFlattenOutput)(responseData, nodeVersion);
            const executionData = this.helpers.constructExecutionMetaData((0, utilities_1.wrapData)(output), {
                itemData: { item: i },
            });
