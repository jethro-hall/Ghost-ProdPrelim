    ...
    methods = {
        loadOptions: {
            ...
            async loadTicketCustomFields() {
                const allFields = await GenericFunctions_1.getAllFields.call(this);
                return (0, GenericFunctions_1.getTicketCustomFields)(allFields).map(GenericFunctions_1.fieldToLoadOption);
            },
            // ----------------------------------
            //          built-in fields
            // ----------------------------------
            ...
            async loadCustomerIds() {
                const users = (await GenericFunctions_1.zammadApiRequest.call(this, 'GET', '/users'));
                return users.filter(GenericFunctions_1.isCustomer).map((i) => ({ name: i.email, value: i.id }));
            },
            async loadGroups() {
                const groups = (await GenericFunctions_1.zammadApiRequest.call(this, 'GET', '/groups'));
            ...
            async loadTicketStates() {
                const states = (await GenericFunctions_1.zammadApiRequest.call(this, 'GET', '/ticket_states'));
                return states.map((state) => ({ name: state.name, value: state.id }));
            },
            async loadTicketPriorities() {
                const priorities = (await GenericFunctions_1.zammadApiRequest.call(this, 'GET', '/ticket_priorities'));
            ...
            try {
                if (resource === 'user') {
                    ...
                    if (operation === 'create') {
                        ...
                        responseData = await GenericFunctions_1.zammadApiRequest.call(this, 'PUT', `/tickets/${id}`, body);
                    }
                    else if (operation === 'delete') {
                        // ----------------------------------
                        //          ticket:delete
