...
exports.getActiveCredentialType = getActiveCredentialType;
...
function getActiveCredentialType(context, itemIndex = 0) {
...
function detectInputFormat(openApiSchema) {
    ...
    if (!invocationUrl) {
        throw new n8n_workflow_1.UserError('No server URL found in OpenAPI schema');
    }
    const pathKeys = Object.keys(openApiSchema.paths);
    if (!pathKeys.length) {
        throw new n8n_workflow_1.UserError('No paths found in OpenAPI schema');
    }
    ...
    if (!postOperation?.requestBody?.content?.['application/json']?.schema) {
        throw new n8n_workflow_1.UserError('No request schema found');
    }
    ...
    switch (detectedFormat) {
        case 'chat':
            if (!requestBody.messages || !Array.isArray(requestBody.messages)) {
                throw new n8n_workflow_1.UserError('Invalid chat format: "messages" array is required');
            }
            break;
        case 'completions':
            if (!requestBody.prompt) {
                throw new n8n_workflow_1.UserError('Invalid completions format: "prompt" is required');
            }
            break;
        case 'embeddings':
            if (!requestBody.input) {
                throw new n8n_workflow_1.UserError('Invalid embeddings format: "input" is required');
            }
            break;
        case 'dataframe_split':
            if (!requestBody.dataframe_split?.data) {
                throw new n8n_workflow_1.UserError('Invalid dataframe_split format: "dataframe_split.data" is required');
            }
            break;
        case 'dataframe_records':
            if (!requestBody.dataframe_records || !Array.isArray(requestBody.dataframe_records)) {
                throw new n8n_workflow_1.UserError('Invalid dataframe_records format: "dataframe_records" array is required');
            }
            break;
        case 'inputs':
            if (!requestBody.inputs) {
                throw new n8n_workflow_1.UserError('Invalid inputs format: "inputs" is required');
            }
            break;
        case 'instances':
            if (!requestBody.instances || !Array.isArray(requestBody.instances)) {
                throw new n8n_workflow_1.UserError('Invalid instances format: "instances" array is required');
            }
            break;
    }
}
