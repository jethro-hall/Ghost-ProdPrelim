...
const getAllowedDomains = (node, credentialData) => {
    if (credentialData.allowedHttpRequestDomains === 'none') {
        throw new n8n_workflow_1.NodeOperationError(node, 'This credential is configured to prevent use within an HTTP Request node');
    }
    if (credentialData.allowedHttpRequestDomains === 'domains') {
        const allowedDomains = credentialData.allowedDomains;
        if (!allowedDomains || allowedDomains.trim() === '') {
            throw new n8n_workflow_1.NodeOperationError(node, 'No allowed domains specified. Configure allowed domains or change restriction setting.');
        }
