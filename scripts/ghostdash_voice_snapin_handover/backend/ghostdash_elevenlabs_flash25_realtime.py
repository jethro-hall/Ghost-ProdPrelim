...
async def flash25_realtime(client_ws: WebSocket) -> None:
    ...
    try:
        ...
        await _send_json_safe(client_ws, {"type": "error", "error": exc.errors(), "metrics": metrics.public()})

    except Exception as exc:
        metrics.status = "failed"
        await _send_json_safe(client_ws, {"type": "error", "error": str(exc)[:1000], "metrics": metrics.public()})

    finally:
        for task in [receive_task, client_task, keepalive_task]:
            if task:
                task.cancel()
