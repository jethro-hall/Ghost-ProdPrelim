...
import { createHubTigerAuthFromEnv } from './lib/hubtiger_auth.js';
import { normalizeAuMobile, orchestrateBookingCreate } from './lib/booking_orchestrator.js';
import {
  bookingSlotHold,
  ...
  bookingSubmit,
  bookingFinalize,
} from './lib/booking_staged_flow.js';
import pg from 'pg';
...
const hubTigerAuth = createHubTigerAuthFromEnv(process.env);
...
async function resolveHubTigerFunctionCode({ forceRefresh = false } = {}) {
  ...
  if (
    !forceRefresh &&
    functionCodeCache.value &&
    now - functionCodeCache.fetchedAt < ttlMs
  ) {
...
export function buildPortalJobSearchRequest({ query, allStores = false, partnerId = HUBTIGER_PARTNER_ID } = {}) {
  ...
  return {
    api: 'services',
    path: '/api/ServiceRequest/JobCardSearch',
    method: 'POST',
    auth: 'bearer',
    payload: {
...
export function mapPortalSearchItem(item) {
  ...
  const closedStatuses = new Set([100, 130]);
  const isOpen = statusCode == null ? true : !closedStatuses.has(statusCode);
  return {
    id: item.ID,
    jobCardNo: item.JobCardNo || item.JobCardID?.toString(),
    customerName: item.CyclistDescription || combinedCustomerName || 'Unknown',
    customerMobile: item.PhoneNumber ?? item.Mobile ?? null,
    ...
    technicianName: item.TechnicianDescription ?? null,
    priceEstimate: item.PriceEstimate ?? null,
    technicianNotes: item.Technician_Notes ?? null,
    assessmentNotes: item.InitialAssesment_Notes ?? null,
    internalNotes: item.PostServiceInspection_Notes ?? null,
  };
}

function formatCyclistDisplayName(cyclist) {
  const composed = normalizeWhitespace(`${cyclist?.Name || ''} ${cyclist?.Surname || ''}`);
  if (composed) return composed;
  return normalizeWhitespace(cyclist?.name || cyclist?.CyclistDescription || '') || 'Unknown';
}
...
function resolveWorkshopName(job) {
  const partner = normalizeWhitespace(job?.PartnerDescription || '');
  if (partner) return partner;
  return inferStoreName(job?.TechnicianDescription || '');
}

function describeActiveWorkshopItem(job) {
  const bike = normalizeWhitespace(job?.BikeDescription || '');
  const serviceType = normalizeWhitespace(job?.TypeDescription || '');
  const statusLabel = mapServiceStatusLabel(Number(job?.StatusID ?? 0) || null, job?.StatusDescription || null);
  return [bike, serviceType, statusLabel].filter(Boolean).join(' — ') || null;
}

function mapOpenJobcardSummary(job) {
  return {
    id: job?.ID ?? null,
    jobCardNo: normalizeJobCardNo(job?.JobCardNo ?? job?.JobCardID ?? ''),
    workshop: resolveWorkshopName(job),
    activeWork: describeActiveWorkshopItem(job),
    bike: normalizeWhitespace(job?.BikeDescription || '') || null,
    serviceType: normalizeWhitespace(job?.TypeDescription || '') || null,
    statusCode: Number(job?.StatusID ?? 0) || null,
    statusLabel: mapServiceStatusLabel(Number(job?.StatusID ?? 0) || null, job?.StatusDescription || null),
    dateCheckedIn: job?.DateCheckedIn ?? null,
    dateBookedIn: job?.DateBookedIn ?? null,
    technicianName: job?.TechnicianDescription ?? null,
  };
}

function buildPhoneSearchVariants(raw) {
  const trimmed = String(raw || '').trim();
  if (!trimmed) return [];
  const digits = trimmed.replace(/\D/g, '');
  const variants = [];
  const auMobile = normalizeAuMobile(trimmed);
  if (auMobile) variants.push(auMobile);
  if (digits.startsWith('61') && digits.length >= 11) {
    variants.push(`+${digits}`);
    variants.push(`0${digits.slice(2)}`);
  }
  if (digits.length === 10 && digits.startsWith('0')) variants.push(digits);
  if (digits.length === 9 && digits.startsWith('4')) variants.push(`0${digits}`);
  if (digits.length >= 9) variants.push(digits);
...
function isActiveWorkshopJob(job) {
  const status = Number(job?.StatusID ?? job?.statusCode ?? 0);
  if (status === 100) return false;
  const label = String(job?.StatusDescription ?? job?.statusLabel ?? '').toLowerCase();
  if (label.includes('collected')) return false;
  return true;
}

function pickPrimaryOpenJobcard(jobs) {
  if (!Array.isArray(jobs) || jobs.length === 0) return null;
  const sorted = [...jobs]
    .filter(isActiveWorkshopJob)
    .sort((a, b) => {
      const aDate = new Date(a?.DateCheckedIn || a?.DateBookedIn || 0).getTime();
      const bDate = new Date(b?.DateCheckedIn || b?.DateBookedIn || 0).getTime();
      return bDate - aDate;
    });
  return sorted[0] || null;
}

function resolveLatestJobForCustomer({ cyclist, searchPhone, openJobRows }) {
  const cyclistId = cyclist?.ID ?? cyclist?.id;
  const phoneKey = phoneMatchKey(searchPhone || cyclist?.PhoneNumber);
  const candidates = [];
  for (const row of openJobRows || []) {
    if (!isActiveWorkshopJob(row)) continue;
    const rowCyclistId = row?.CyclistID ?? row?.cyclistId;
    const rowPhoneKey = phoneMatchKey(row?.PhoneNumber);
    if (cyclistId != null && rowCyclistId != null && String(rowCyclistId) === String(cyclistId)) {
      candidates.push(row);
      continue;
    }
    if (phoneKey && rowPhoneKey && phoneKey === rowPhoneKey) candidates.push(row);
  }
  return pickPrimaryOpenJobcard(candidates);
}

function buildCustomerSearchVariables(cyclist, jobcardSummary) {
  const name = formatCyclistDisplayName(cyclist);
  if (!jobcardSummary) {
    return {
      Name: name,
      Jobcard: '',
  ...
  return {
    Name: name,
    Jobcard: jobcardSummary.jobCardNo || '',
    Model: jobcardSummary.bike || '',
    Workshop: jobcardSummary.workshop || '',
    Location: jobcardSummary.workshop || '',
    WorkshopActive: jobcardSummary.activeWork || '',
    DateCheckedIn: jobcardSummary.dateCheckedIn || jobcardSummary.dateBookedIn || '',
  };
}

function enrichCyclistResult(cyclist, { searchPhone, openJobRows, fallbackJobcard = null }) {
  const primaryJob = resolveLatestJobForCustomer({ cyclist, searchPhone, openJobRows });
  const jobcard = primaryJob
    ? mapOpenJobcardSummary(primaryJob)
    : (fallbackJobcard || null);
  return {
    ...cyclist,
    name: formatCyclistDisplayName(cyclist),
    jobcard,
  };
}

async function fetchJobCardSearchMatches(query, allStores = true) {
  const search = String(query || '').trim();
  if (!search || !HUBTIGER_PARTNER_ID) return [];
  const { response, data } = await portalFetch({
    api: 'services',
    path: '/api/ServiceRequest/JobCardSearch',
    method: 'POST',
    body: { PartnerID: Number(HUBTIGER_PARTNER_ID), Search: search, SearchAllStores: allStores === true },
    auth: 'bearer',
  });
...
async function fetchOpenWorkshopJobcards() {
  const path = `/api/Partner/${encodeURIComponent(HUBTIGER_PARTNER_ID)}/JobcardsMinimumV2/open/${encodeURIComponent(HUBTIGER_PARTNER_ID)}`;
  const { response, data } = await portalFetch({
    api: 'services',
    path,
    method: 'GET',
  ...
  if (!response.ok) {
    return { ok: false, rows: [], upstream_status: response.status, data };
  }
  const rows = Array.isArray(data) ? data : (data?.results || data?.data || []);
  return { ok: true, rows };
}

async function fetchOpenWorkshopJobcardsBounded(timeoutMs = 8000) {
  let timer = null;
  try {
    return await Promise.race([
      fetchOpenWorkshopJobcards(),
      new Promise((resolve) => {
        timer = setTimeout(() => resolve({ ok: false, rows: [], timedOut: true }), timeoutMs);
      }),
    ]);
  } finally {
...
async function portalSearchCyclistsUpstream({ search, type, page, limit }) {
  const path = `/api/Bikeshop/${encodeURIComponent(HUBTIGER_PARTNER_ID)}/Search/Cyclists`;
  const { response, data } = await portalFetch({
    api: 'api',
    path,
    method: 'GET',
    query: { Page: page, Limit: limit, Search: search, Type: type },
    auth: 'bearer',
  });
  if (!response.ok) {
    return { ok: false, upstream_status: response.status, data, rows: [] };
  }
  const rows = Array.isArray(data)
    ? data
    : (data?.cyclists || data?.results || data?.data || []);
  return { ok: true, rows };
}

async function resolveCyclistRowsForQuery(q, type, page, limit) {
  const isPhone = type.toLowerCase() === 'phone';
  const queries = isPhone ? buildPhoneSearchVariants(q) : [q];
  let lastError = null;
  for (const search of queries) {
    const result = await portalSearchCyclistsUpstream({ search, type, page, limit });
    if (!result.ok) {
      lastError = result;
      continue;
    }
    if (result.rows.length > 0) {
      return { ok: true, rows: result.rows, searchUsed: search };
    }
  }
  if (lastError && !lastError.ok) return lastError;
  return { ok: true, rows: [], searchUsed: queries[0] || q };
}
...
async function fetchProductsCatalogFromUpstream() {
  ...
  let availabilityPayload = data;

  // HubTiger sometimes returns JSON as a JSON string rather than a real array/object.
  // Example shapes:
  //   []
  //   "[{...}]"
  //   "{\"data\":[...]}"
  for (let i = 0; i < 3 && typeof availabilityPayload === 'string'; i += 1) {
    const trimmed = availabilityPayload.trim();
    if (!trimmed) break;
    try {
      availabilityPayload = JSON.parse(trimmed);
    } catch {
      break;
    }
  }
  ...
  const rows =
    Array.isArray(availabilityPayload) ? availabilityPayload :
    Array.isArray(availabilityPayload?.data) ? availabilityPayload.data :
    Array.isArray(availabilityPayload?.rows) ? availabilityPayload.rows :
    Array.isArray(availabilityPayload?.availability) ? availabilityPayload.availability :
    Array.isArray(availabilityPayload?.availabilities) ? availabilityPayload.availabilities :
    Array.isArray(availabilityPayload?.slots) ? availabilityPayload.slots :
    Array.isArray(availabilityPayload?.technicians) ? availabilityPayload.technicians :
    [];
  return {
    ok: true,
    status: response.status,
...
async function portalFetch({
  ...
  const url = api === 'services' ? portalServicesUrl(path, query) : await portalApiUrlAsync(path, query);
...
function findEarliestAvailability(rows, requiredMinutes = 60, now = new Date(), options = {}) {
  const candidates = findAllAvailabilityCandidates(rows, requiredMinutes, now, options);
  return candidates.length ? candidates[0] : null;
}

function openSlotsFromCandidates(candidates) {
  return candidates.map((candidate) => ({
    availableSlot: candidate.slotStart,
...
async function portalSearchJobs(req, res) {
  ...
  if (!response.ok) {
    const txt = typeof json?._raw === 'string' ? json._raw : JSON.stringify(json || {});
...
async function portalTechnicianAvailability(req, res) {
  ...
  if (!technicians && store === 'brisbane') {
    const configured = String(process.env.HUBTIGER_BRISBANE_AVAILABILITY_TECHNICIANS || '2730,6744').trim();
    if (configured) technicians = configured;
  }
  if (!technicians) {
  ...
  const list = rows.slice(0, Math.max(1, MAX_AVAILABILITY_ROWS));
  const candidates = findAllAvailabilityCandidates(list, requiredMinutes, new Date(), { store });
  const earliest = candidates.length ? candidates[0] : null;
  const openSlots = openSlotsFromCandidates(candidates);
  return res.json({
    ok: true,
    store,
    rows: list,
    ...
    requiredMinutes,
    earliest,
    upstreamShape: Array.isArray(data) ? 'array' : typeof data,
    parsedShape: Array.isArray(availabilityPayload) ? 'array' : typeof availabilityPayload,
    upstreamKeys: data && typeof data === 'object' && !Array.isArray(data) ? Object.keys(data).slice(0, 30) : [],
    parsedKeys: availabilityPayload && typeof availabilityPayload === 'object' && !Array.isArray(availabilityPayload) ? Object.keys(availabilityPayload).slice(0, 30) : [],
    upstreamStringLength: typeof data === 'string' ? data.length : null,
    upstreamStringSample: typeof data === 'string' ? data.slice(0, 500) : null,
    parsedStringSample: typeof availabilityPayload === 'string' ? availabilityPayload.slice(0, 500) : null,
  });
}
...
async function portalSearchCyclists(req, res) {
  ...
  const [cyclistResult, openJobsResult] = await Promise.all([
    resolveCyclistRowsForQuery(q, type, page, limit),
    fetchOpenWorkshopJobcardsBounded(8000).catch(() => ({ ok: false, rows: [] })),
  ]);
  if (!cyclistResult.ok) {
    return res.status(502).json({
      ok: false,
      error: 'portal_customer_search_failed',
      upstream_status: cyclistResult.upstream_status,
      data: cyclistResult.data,
    });
  }
  const rows = cyclistResult.rows;
  const openJobRows = openJobsResult.ok ? openJobsResult.rows : [];

  let fallbackJobcard = null;
  if (rows.length > 0) {
    const latestOpen = resolveLatestJobForCustomer({ cyclist: rows[0], searchPhone: q, openJobRows });
    if (!latestOpen) {
      try {
        const searchQueries = type.toLowerCase() === 'phone' ? buildPhoneSearchVariants(q) : [q];
        let searchMatches = [];
        for (const searchQuery of searchQueries) {
          searchMatches = await fetchJobCardSearchMatches(searchQuery, true);
          if (searchMatches.length > 0) break;
        }
        const phoneKey = phoneMatchKey(q);
        const filtered = searchMatches.filter((row) => {
          if (!isActiveWorkshopJob(row)) return false;
          if (!phoneKey) return true;
          return phoneMatchKey(row?.PhoneNumber) === phoneKey;
        });
        const latestSearch = pickPrimaryOpenJobcard(filtered.length > 0 ? filtered : searchMatches.filter(isActiveWorkshopJob));
        if (latestSearch) fallbackJobcard = mapOpenJobcardSummary(latestSearch);
      } catch (_) {}
    }
  }

  const results = rows.map((cyclist, index) => enrichCyclistResult(cyclist, {
    searchPhone: q,
    openJobRows,
    fallbackJobcard: index === 0 ? fallbackJobcard : null,
  }));
  const primary = results[0] || null;
  const variables = primary
    ? buildCustomerSearchVariables(primary, primary.jobcard)
    : { Name: '', Jobcard: '', Model: '', Workshop: '', Location: '', WorkshopActive: '', DateCheckedIn: '' };

  return res.json({ ok: true, results, count: results.length, variables });
}
...
async function runStagedBookingStep(handler, req, res) {
  const payload = req.body && typeof req.body === 'object' ? req.body : {};
  try {
    hubTigerAuth.partnerId = HUBTIGER_PARTNER_ID || hubTigerAuth.partnerId;
    const result = await handler({
      auth: hubTigerAuth,
      payload,
      partnerId: HUBTIGER_PARTNER_ID,
      createdByUserId: HUBTIGER_CREATED_BY_USER_ID,
      sendCommunication: req.query.sendCommunication !== 'false',
    });
    if (!result.ok) {
      return res.status(result.status || 400).json({
        ok: false,
        message: result.assistant_prompt || result.hint || 'Booking step could not be completed.',
        data: result,
        ...result,
      });
    }
    return res.json({
      ok: true,
      message: result.assistant_prompt || 'Step completed.',
      data: result,
      ...result,
    });
  } catch (err) {
    return res.status(502).json({
      ok: false,
      error: 'booking_staged_step_failed',
...
async function portalScheduleService(req, res) {
  ...
  try {
    hubTigerAuth.partnerId = HUBTIGER_PARTNER_ID || hubTigerAuth.partnerId;
    const result = await orchestrateBookingCreate({
      auth: hubTigerAuth,
      payload,
      sendCommunication,
      partnerId: HUBTIGER_PARTNER_ID,
    ...
    if (!result.ok) {
      return res.status(result.status || 502).json({
        ok: false,
        error: result.error || 'portal_schedule_service_failed',
    ...
    return res.json({
      ...
      customer_outcome: result.customer_outcome,
...
app.get('/availability/technicians', (req, res) => {
  if (PORTAL_MODE || HUBTIGER_PARTNER_ID) return portalTechnicianAvailability(req, res).catch((e) => res.status(502).json({ ok: false, error: 'portal_availability_failed', message: String(e && e.message || e) }));
...
app.post('/booking/session/slot', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingSlotHold, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/customer/search', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingCustomerSearch, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/customer/confirm', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingCustomerConfirm, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/bike/list', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingBikeList, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/bike/confirm', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingBikeConfirm, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/service', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingServiceSet, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/submit', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingSubmit, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});

app.post('/booking/session/finalize', (req, res) => {
  if (PORTAL_MODE) return runStagedBookingStep(bookingFinalize, req, res);
  return res.status(400).json({ ok: false, error: 'booking_staged_only_supported_in_portal_mode' });
});
